...
<div style="width: 80%;">Description/Demo</div>
    ...
    <div style="border: 0.5px solid silver;display: flex;justify-content: space-between;flex-direction: row;width: 50%;margin: auto;padding: 20px;">
        ...
        <div style="width: 20%;">${{$invoice->job_bid_amount}}</div>
    </div>
    @endif
    <div style="border: 0.5px solid silver;display: flex;justify-content: space-between;flex-direction: row;width: 50%;margin: auto;padding: 20px;">
        <div style="width: 80%;">Charges</div>
        <div style="width: 20%;">${{$invoice->client_pay_to_admin-$invoice->job_bid_amount}}</div>
    </div>
    <div style="border: 0.5px solid silver;display: flex;justify-content: space-between;flex-direction: row;width: 50%;margin: auto;padding: 20px;">
        <div style="width: 80%;">Total Payment</div>
        <div style="width: 20%;">$@if($role=='client'||$role=='admin'){{$invoice->client_pay_to_admin}}@else  {{$invoice->job_bid_amount}}   @endif</div>
    </div>
    @if($role=='client')
        <div style="border: 0.5px solid silver;display: flex;justify-content: space-between;flex-direction: row;width: 50%;margin: auto;padding: 20px;">
            <div style="width: 80%;">{{$job->job_title}}</div>
            <div style="width: 20%;">${{$invoice->client_pay_to_admin}}</div>
        </div>
    @endif
    ...
    @if($role=='admin')
        <div style="border: 0.5px solid silver;display: flex;justify-content: space-between;flex-direction: row;width: 50%;margin: auto;padding: 20px;">
            ...
            <div style="width: 20%;">${{$invoice->client_pay_to_admin-$invoice->amount}}</div>
        </div>
    @endif
