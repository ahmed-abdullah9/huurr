    ...
    @if(!empty($frcomment))
        <div class="row">
            <div style="z-index: 0;" class="col-md-offset-2 col-md-8">
                <h3 align="center">Freelancer Given Feedback <small>on</small></h3>
                <h4 align="center">@if(isset($clcomment->job_title))
                        {{$clcomment->job_title}}
                    @endif</h4>
                ...
                <label for="experience2">{{$frcomment->experience}}</label>
                <h3 style="font-size: 20px;">Response Time</h3>
                <div  id="timing_response2"></div>
                <label for="timing_response2">{{$frcomment->response_time}}</label>
                <h3 style="font-size: 20px;">Communication Experience</h3>
                <div  id="communication_experience2"></div>
                <label for="communication_experience2">{{$frcomment->com_experience}}</label>
                <h3 style="font-size: 20px;">Technical Skills</h3>
                <div  id="technical_skills2"></div>
                <label for="technical_skills2">{{$frcomment->tech_skills}}</label>
                <hr>
                <div>
                </div>
            </div>
  ...
  @if(empty($clcomment))
    <div class="row">
      <div style="padding-bottom: 10%;z-index: 0;" class="col-md-offset-2 col-md-8">
        <form style="width: 100%;" action="{{url('submit/cl/feedback/')}}" method="post">
          <input type="hidden" id="proposal_id" name="proposal_id" value="{{encrypt($for_feedback->proposal_id)}}">
          <input type="hidden" id="feedback_by" name="feedback_by" value="{{encrypt(1)}}">
  ...
  @else
    <div class="row">
      <div style="padding-bottom: 10%;z-index: 0;" class="col-md-offset-2 col-md-8">
        ...
        <label for="experience1">{{$clcomment->experience}}</label>
        <h3 style="font-size: 20px;">Response Time</h3>
        <div  id="timing_response1"></div>
        <label for="timing_response1">{{$clcomment->response_time}}</label>
        <h3 style="font-size: 20px;">Communication Experience</h3>
        <div  id="communication_experience1"></div>
        <label for="communication_experience1">{{$clcomment->com_experience}}</label>
        <h3 style="font-size: 20px;">Technical Skills</h3>
        <div  id="technical_skills1"></div>
        <label for="technical_skills1">{{$clcomment->tech_skills}}</label>
        <hr>
          @if($clcomment->progress==2)
          <button type="button" class="btn btn-round btn-primary btn-color" onclick="post_to_complete(<?php echo $clcomment->proposal_id; ?>, <?php echo $clcomment->user_id; ?>)">Mark To Complete</button>
        @endif
              <div>
        </div>
      </div>
