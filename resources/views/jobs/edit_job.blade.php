...
<form action="{{ url('jobpost/update') }}" id="postjobform" method="POST" enctype="multipart/form-data">
  <input type="hidden" name="job_id" value="{{ $jobs->job_id }}">
	{{ csrf_field() }}
        <div style="padding-bottom: 10px;" class="row clienttoprow">
          ...
          <div class="col-md-12 col-sm-12">
            <h3  class="main-form-title project-title">{{__('client.edit_job')}}</h3>
          </div>
        </div>
                  ...
                  <select name="category_id" onchange="view_skills(this.value)" class="form-control" required style="color: #000;">
                   ...
                   <div class="form-group">
                       ...
                       <select style="width: 30% !important;" id="job_skills" name="job_skills[]" multiple="multiple">
                           <option value="" disabled>{{__('client.select_job_skills')}}</option>
                           @if(sizeof($skills)>0)
                   ...
                   <div style="@if($jobs->job_time_type!='fixed') display:none; @endif" class="budget form-group">
                       <h4 class="form-label-color">{{__('client.budget')}} <span class="red">*</span></h4>
